...
def _getBasePairing(dir, j, vec, se, i, rsp, threadLimiter):
    ...
    try:
        ...
        cmd = 'partition ' + dir + 'temp_'+str(j+1)+'.seq ' + dir + 'temp_'+str(j+1)+'.pfs'+rsp
        ...
        cmd = 'ProbabilityPlot ' + dir + 'temp_'+str(j+1)+'.pfs ' + dir + 'temp_'+str(j+1)+'.txt -t'
        subprocess.check_output(cmd, shell=True)
...
#Output: 1D array
##########################################################################################################
def getMapSeqs(dir, fasta_file, n, rg, rsp, thmax=1):
    ...
    if(len(ind)<n):
        cmd = 'rm -f ' + dir + '*'
        subprocess.check_output(cmd, shell=True)
        sys.exit("Map cannot be created. Not enough map sequences are properly folding.")
...
#Output: db file
##########################################################################################################
def getMapStruct(dir, mapinds, outfile):
    ...
    #print update to standard out
    print("Generating structures for map of conformational space...")

    #loop through each of map sequences
    for i in mapinds:
        
        #runs stochastic without SHAPE contraints
        cmd = 'stochastic ' + dir + 'temp_'+str(i)+'.pfs ' + dir + 'temp.con -e 1000'
        subprocess.check_output(cmd, shell=True)
